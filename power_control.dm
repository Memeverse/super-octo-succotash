...
//		src.CurPcnt = C
	//		ASSERT(e)
		if(playerID.client)  //NULL VAR BUG FIX  -  Attempt to remove PC Drain calls.    Updated to add it back in -  2013 then taken out -  BACK IN AGAIN, WOOHOO 9/5/2013
			spawn(1) playerID.PC_Drain(src.playerpower,40) //NULL VAR BUG CRASH  Was src.player,40  |  Switched back to src.player due to mob call issues.
...
//		if(isnull(playerID)){log_errors("PowerControl Timer did not end properly, ending it now.");skills_scheduler.cancel(src);return} // Cancels itself when it has no player. Updated to add it back in to remove null calls in errorlogs - 2013
		if(isnull(playerID)){skills_scheduler.cancel(src);return} // Goes hand in hand with making sure it dies when it's supposed to.
		...
		//Power Control
		for(var/obj/Power_Control/A in src.playerID.contents)
			if(src.playerID.icon_state=="KO"&&A.Powerup)
				A.Powerup=0
				src.playerID.AuraOverlays()
				src.playerID.Cancel_PowerControl(src)
				src.playerID=null // sometimes Cancel_PowerControl doesn't properly kill the scheduler and we REALLY want it to DIE.
			if(A.Powerup&&A.Powerup!=-1)
				if(src.playerID.icon_state=="Meditate"&&src.playerID.BPpcnt>=99){src.playerID.BPpcnt=100;return}
				else src.playerID.BPpcnt += 1*src.playerID.Recovery
			else if(A.Powerup==-1) src.playerID.BPpcnt*=0.9
...
// Above 100% Drain
		...
		if(src.playerID.icon_state!="KO"&&src.playerID.BPpcnt>100)
			var/Drain=1*(src.playerID.BPpcnt-100)/pick(1,src.playerID.Recovery)
			if(src.playerID.Ki>=Drain*10) src.playerID.Ki-=Drain
			else
				src.playerID.BPpcnt=100
				for(var/obj/Power_Control/A in src.playerID.contents) A.Powerup=0
				if(src.playerID.Race in list("Beastman","Saiyan","Half-Saiyan")) src.playerID.Cancel_Transformation()
				src.playerID << "You are too tired to hold the energy you gathered, your energy levels return to normal."
				src.playerID.Cancel_PowerControl()
				src.playerID = null
...
mob/proc/PC_Drain(var/mob/player,var/sleeptime=20)
	if(isnull(player) || isnull(player.client) ) return
