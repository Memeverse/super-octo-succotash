		...
		verb
			Enchant()
				...
				if(src.Builder == usr.ckey)
					switch(input("Are you sure you want to enchance this door with your magic? In doing so, it will cost 100,000,000 mana, but protect this door from being hacked by a tech console.") in list("No","Yes"))
						if("Yes")
								...
								for(var/obj/Mana/M in usr)
									if(M.Value >= 100000000)
										...
										for(var/mob/X in range(20,src))
											if(X.client)
												X.saveToLog("| [usr.client.address ? (usr.client.address) : "IP not found"] | ([usr.x], [usr.y], [usr.z]) | [key_name(usr)] enchances their door [src].\n")
										src.Magic_Secure = 1
			Remove()
				set src in range(1,usr)
				if(src.Builder == usr.ckey)
						...
						if("Yes")
							if(usr in range(1,src))
								...
								for(var/mob/M in range(20,src))
									if(M.client)
										M.saveToLog("| [usr.client.address ? (usr.client.address) : "IP not found"] | ([usr.x], [usr.y], [usr.z]) | [key_name(usr)] removed their door [src].\n")
								del(src)
		proc
			Open()
