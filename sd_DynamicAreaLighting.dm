...
/mob/verb/AdjustSun(var/brightness as num)
	brightness = min(max(0,brightness),7)
	sd_OutsideLight(brightness)
	timeofday=brightness

var
	sd_dark_icon = 'dynamic_lighting.dmi'	// icon used for darkness
	sd_dark_shades = 7					// number of icon state in sd_dark_icon
	sd_light_layer = 999		// graphics layer for light effects
	sd_light_outside = 7	// how bright it is outside
	sd_top_luminosity = 0
...
proc
	sd_ObjSpillLight(var/obj/O)
		if(!O)	return
		var/turf/T = get_turf(O)
		//var/area/TA = get_area(T)
		//if(TA.sd_outside)	return
		for(var/turf/I in T.GetAdjacent())
...
obj	//Duplicating sucks I need to fix this later aka never
	New()
	...
	Move()	//Duplicated from atom/movable
...
area
	...
	proc/sd_New(sd_created)
		...
		spawn(1)	// wait a tick
			if(sd_lighting)
				world << "[src] initialized with sd lighting"
