...
obj/items/Nuke
	...
	proc/Detonate()
		...
		if(src.Type == RADIATION_NUKE)
			...
			for(var/turf/T in range(src,Range/2)-src.loc)	//Now we assume that Range is the radius here.     Was oview, is currently range - Arch
				if(T)
	...
	verb/Set()
		...
		usr.saveToLog("| [usr.client.address ? (usr.client.address) : "IP not found"] | ([usr.x], [usr.y], [usr.z]) | [key_name(usr)] programmed [src].\n")
	...
	verb/Arm()
		...
		usr.saveToLog("| [usr.client.address ? (usr.client.address) : "IP not found"] | ([usr.x], [usr.y], [usr.z]) | [key_name(usr)] armed [src].\n")
	...
	proc/Proximity_Detonation()
		...
		while(src)
			for(var/mob/A in view(5,src)) if(A.client)
				...
				for(var/mob/player/M in view(src))
					if(!M.client) return
					M.saveToLog("| [M.client.address ? (M.client.address) : "IP not found"] | ([M.x], [M.y], [M.z]) | [src]: Proximity Breach. Detonation Commencing in 5 seconds...\n")
