...
obj/Faction
	...
	Click()
		...
		switch(input("Choose Option") in Choices)
			...
			if("View Faction Notes")
				usr<<browse(notes,"window= ;size=700x600")
			if("Members Online") for(var/mob/player/M in Players) for(var/obj/Faction/A in M)
				if(A.factioncode==factioncode)
					if(!M.Dead) usr<<"[M]"
					else usr<<"[M] (Dead)"
mob/proc/FactionUpdate() for(var/obj/Faction/F in src) for(var/mob/player/M in view(src))
