...
mob/proc/Michael_Jackson_Dance()
	...
	icon='Michael Jackson.dmi'
	icon_state="1"
	for(var/mob/B in view(src)) if(!(locate(/obj/Michael_Jackson) in B)) B.Michael_Jackson_Dance()
	spawn(160) if(src)
		icon_state=""
		Un_KO()
...
proc/MJ_Dance()
	...
	while(1)
		...
		for(var/mob/A in Players) if((locate(/obj/Michael_Jackson) in A)&&A.icon_state!="1")
			if(A&&A.icon_state=="KO") spawn if(A) A.Un_KO()
			step(A,Direction)
...
obj/Michael_Jackson
	New()
		...
		//..()
//	Del()
//		for(var/mob/A in Players) for(var/obj/Michael_Jackson/B in A) if(B==src)
//			A.icon=icon
//			if(overlays) A.overlays+=overlays
//		..()
mob/proc/Dance_Effects()
	while(locate(/obj/Michael_Jackson) in src)
		...
		if(src)
			...
			if(A==3)
				var/list/B=new
				for(var/turf/C in view(12,src)) if(!(locate(typesof(/mob)) in C)) B+=C
