...
proc/Mob_Range(atom/A,range) if(A)
	...
	//	if(!ismob(B)) return Sanity check? Apparently, while looping through the list it can sometimes STILL be null?
		...
		if(B.client) if (A) //Second sanity check that I added in.  - Arch
			if(B.z==A.z&&B.x>=startx&&B.x<=endx&&B.y>=starty&&B.y<=endy)
				var/Max_Distance=((B.BP-B.Artificial_Power)/1000)+100 //The max distance this individual can be sensed from.
				if(B.Artificial_Power>B.BP*10) Max_Distance=0
				if(Max_Distance)
